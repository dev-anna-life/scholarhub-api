...
function getTransporter() {
  ...
  const user = process.env.EMAIL_USER || process.env.SMTP_USER || 'scholarhubng1@gmail.com'
  const rawPass = process.env.EMAIL_PASS || process.env.SMTP_PASS || 'zaew jokd rqdy uztq'
...
async function sendVerificationEmail(toEmail, otpCode) {
  ...
  const from = process.env.EMAIL_FROM || '"ScholarHub Security" <scholarhubng1@gmail.com>'
